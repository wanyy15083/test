...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('test-systemid', systemid);
		$.cookie('test-systemname', systemname);
		$.cookie('test-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('test-systemid') || 1;
	var systemname = $.cookie('test-systemname') || 'test';
	var systemtitle = $.cookie('test-systemtitle') || '后台管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
